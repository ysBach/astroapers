...
class ApMask:
    ...
    Compatibility wrapper around ``BoundingBox`` methods. The sampled weights
    remain available as ``.weights`` and the placement remains available as
    ``.bbox``.
    """

    def __init__(self, weights, bbox: BoundingBox) -> None:
        self.bbox = bbox
        self.weights = _validate_weights(weights, bbox)

    def npix(self, shape: tuple[int, int], mask=None) -> float:
        """Return the in-frame sum of mask weights for ``shape``."""
        return self.bbox.npix(self.weights, shape, mask=mask)
...
def apmask_to_image(
    ...
    """Embed bbox-tight mask weights in a full image array."""
    return bbox.to_image(weights, shape, fill_value=fill_value)
...
def apmask_weighted_cutout(
    ...
    return bbox.weighted_cutout(weights, image, mask=mask, fill_value=fill_value)
...
def apmask_weighted_values(weights, bbox: BoundingBox, image, mask=None) -> np.ndarray:
    ...
    return bbox.weighted_values(weights, image, mask=mask)


def apmask_npix(weights, bbox: BoundingBox, shape: tuple[int, int], mask=None) -> float:
    """Return the in-frame sum of raw aperture weights for ``shape``."""
    return bbox.npix(weights, shape, mask=mask)
...
def apmask_apsum(
    ...
    """Return aperture sum, and npix by default, for raw aperture weights."""
    return bbox.apsum(weights, image, mask=mask, return_npix=return_npix)
