...
For custom composed masks, the raw Rust extension can generate the component
bbox-tight weights without constructing aperture objects. Import it as `aapr`.
The composition itself is still your Python logic; there is no raw fused Rust
function for an arbitrary user-composed shape. For more raw-call patterns,
inspect `astroapers.kernels`.
