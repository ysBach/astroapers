...
`astroapers` is also usable as a Rust crate. The public Rust aperture helpers do
not depend on PyO3 or NumPy unless the Python extension feature is enabled.
...
For a local checkout before the crate is published, use:

```toml
[dependencies]
astroapers = { path = "../astroapers", default-features = false }
```

```rust
use astroapers::{circle_bbox, circle_exact_weight_at_pixel};

let bbox = circle_bbox(8.0, 8.0, 3.0);
let mut area = 0.0;

for y in bbox.y_range() {
    for x in bbox.x_range() {
        area += circle_exact_weight_at_pixel(8.0, 8.0, 3.0, x, y);
    }
}

assert!((area - std::f64::consts::PI * 9.0).abs() < 1.0e-12);
```

The current Rust-facing API exposes bbox-tight extents and exact pixel weights
for circles, ellipses, and rectangles. Build local Rust API docs with:
...
- [Rust API](rust-api.qmd), a landing page for the generated rustdoc output.
